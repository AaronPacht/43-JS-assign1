var transactions = [
 ...
 transactions.forEach(function(transaction){if(transaction.type==="purchase"){numPurchases++};});
...
transactions.forEach(function(transaction){if(transaction.type==='sale' && transaction.paymentMethod==='cash'){numCashSales++};});
console.log(numCashSales);
...
transactions.forEach(function(transaction){if(transaction.type==='purchase' && transaction.paymentMethod==='credit'){creditPurchases++};});
...
uniqueVendors=transactions
  .filter(transaction => {return transaction.vendor})
  .map(transaction => {if (transaction.vendor){return transaction.vendor}});
...
/*
   ...
   uniqueCustomers=transactions
    .filter(transaction => {return transaction.customer})
    .map(transaction => {if (transaction.customer){return transaction.customer}});
  let unique=[]
  uniqueCustomers.forEach(customer=>{if(!unique.find(c=>c===customer)){unique.push(customer);}});
  console.log(unique);
 ...
 let bigSpenders=transactions
  .filter(transaction=>{return transaction.items.length>=5})
  .map(transaction=>{if(transaction.customer){title=transaction.customer}else{title=transaction.vendor}; return { name: title, numItems: transaction.items.length }});
 ...
 */
let sumSales=transactions
  .filter((transaction,index)=>{return transaction.type==='sale'})[0]
...
let sumPurchases=transactions
  .filter((transaction,index)=>{return transaction.type==='purchase'})
  .map(transaction=>{return transaction.items.map(item=>{return item.price}).reduce((total,price)=>{return total+price})})
  .reduce((total,price)=>{return total+price})
...
let mostItems=transactions
  .map(transaction=>{return transaction.items.length})
...
console.log(mostItems);
  ...
  */
let least=transactions
  .filter(transaction=>{return transaction.type==='purchase'})
  .map(transaction=>{return transaction.items.length})
  .sort((a,b)=>{return a-b})[0];
let sumOfSmallestPurchase=transactions
  .filter(transaction=>{return transaction.type==='purchase' && transaction.items.length===least})[0]
  .items.map(item=>{return item.price})
  .reduce((total,price)=>{return total+price});
